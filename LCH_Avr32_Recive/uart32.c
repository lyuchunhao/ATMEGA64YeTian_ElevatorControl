...
void Uart_Init(void)
{
	   UCSRB = 0x00;
	   UCSRA = 0x00;                 //指示相关状态
	   UCSRC = 0x06;                 //（UCSZ1：UCSZ0）Bit2-1 字符长度8位，结合UCSRB中的UCSZ2=0（Bit2）
	   UBRRL = 0x4D;                 //12M晶振，波特率=9600bps
	   UBRRH = 0x00;
	   UCSRB = 0x98;                 //（RXCIE）Bit7=1接收中断使能，（RXEN）Bit4=1接收使能，（TXEN）Bit3=1发送使能
}
/***************************************************************
Copyright(C), 2013.08.11  Sunny Studio
作者           ：LCH_shadow 
...
{
	   while(!(UCSRA&0x20));        //UDRE=1 数据寄存器已空
	   UDR = data;
}
/***************************************************************
Copyright(C), 2013.08.11  Sunny Studio
...
{
	   while(!(UCSRA&0x80));        //RXC=1 等待接收数据
	   return UDR;                  //从缓冲区获取并返回数据
}   
/***************************************************************
Copyright(C), 2013.08.11  Sunny Studio
作者           ：LCH_shadow 
